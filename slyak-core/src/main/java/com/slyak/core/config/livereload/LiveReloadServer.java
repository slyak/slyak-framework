...
package com.slyak.core.config.livereload;
		...
		do {
			try {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("LiveReloadConfig server error", ex);
				}
			}
		}
	...
	private class ConnectionHandler implements Runnable {
		...
		public void run() {
			try {
				...
				logger.debug("LiveReloadConfig connection closed");
			}
			catch (Exception ex) {
				if (logger.isDebugEnabled()) {
					logger.debug("LiveReloadConfig error", ex);
				}
			}
		}
